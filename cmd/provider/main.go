...
import (
	...
	apisCluster "github.com/facilitygrid/provider-signoz/apis/cluster"
	apisNamespaced "github.com/facilitygrid/provider-signoz/apis/namespaced"
	"github.com/facilitygrid/provider-signoz/config"
	"github.com/facilitygrid/provider-signoz/internal/clients"
	controllerCluster "github.com/facilitygrid/provider-signoz/internal/controller/cluster"
	controllerNamespaced "github.com/facilitygrid/provider-signoz/internal/controller/namespaced"
	"github.com/facilitygrid/provider-signoz/internal/features"
	"github.com/facilitygrid/provider-signoz/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for SigNoz").DefaultEnvars()
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped SigNoz APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced SigNoz APIs to scheme")
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped SigNoz controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced SigNoz controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped SigNoz controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced SigNoz controllers")
	}
